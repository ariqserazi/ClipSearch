...
from app.routers import agent, clips, collect, health, reports, ui
...
input, select { width:100%; min-height:38px; border-radius:8px; border:1px solid var(--line); background:var(--panel2); color:var(--text); padding:7px 9px; }
...
@media (max-width: 760px) { .stats { grid-template-columns:repeat(2, minmax(0, 1fr)); } .filters { grid-template-columns:1fr; } .clip-card { grid-template-columns:1fr; } .shell { width:min(100% - 20px, 1180px); } }
"""
